...
struct XDRControllerTests {
    ...
    ) -> (XDRController, StubDisplayServices, StubNightShiftManager, StubGammaTableManager) {
    ...
    func enableXDR_unsupported_returnsFalse() {
        let (controller, _, _, _) = makeController(supportsXDR: false)
    ...
    func enableXDR_capturesHardwareBrightness() {
        ...
        let (controller, _, _, _) = makeController(displayServices: ds)
    ...
    func enableXDR_maxesBrightnessAndDisablesALC() {
        let (controller, ds, _, _) = makeController()
        _ = controller.enableXDR()
    ...
    func enableXDR_disablesNightShift() {
        ...
        let (controller, _, _, _) = makeController(nightShift: ns)
        _ = controller.enableXDR()
    ...
    func disableXDR_stopsGammaLoop() {
        let (controller, _, _, gamma) = makeController()
        _ = controller.enableXDR()
    ...
    func disableXDR_restoresNightShiftOnlyIfWasEnabled() {
        ...
        let (controllerA, _, _, _) = makeController(nightShift: nsOff)
        ...
        let (controllerB, _, _, _) = makeController(nightShift: nsOn)
